...
# Phase 2: Detection Efficacy & Evasion Analysis

## 1. Objective
The objective of Phase 2 is to evaluate the detection capabilities of the OWASP Core Rule Set (v3.3) running at Paranoia Level 2, and to identify structural "Parser Differentials"—scenarios where the WAF normalizes input differently than the backend application (DVWA/MySQL), resulting in a security bypass.

---

## 2. Baseline Efficacy (True Positives)

Before attempting evasion, a baseline of standard attack vectors was established using an automated PyTest/Requests script against the `/login.php` endpoint. 
...
**Conclusion:** The failure to block isolated keywords like `select` or `update` is not a flaw in ModSecurity, but a necessary design choice to prevent catastrophic **False Positives**. If the WAF blocked every request containing the word "select", standard web functionality (e.g., a user typing "I will select an option") would break. The CRS requires *syntax context* (e.g., a string breakout followed by an operator and a keyword) to trigger a block.

---

## 3. Evasion Methodologies (Parser Differentials)

To bypass the baseline defenses, testing pivoted from brute-force signatures to targeting the **Normalization Gap**—exploiting how ModSecurity decodes data compared to how PHP/Apache decodes data.

### 3.1 Vector 1: MySQL Versioned Comment Evasion (Parser Differential)
...
![[Pasted image 20260528231632.png]]
...
**Conclusion:** While the request was technically blocked by a PL2 protocol rule, the test was a success. It proved that ModSecurity's core SQL syntax engines can be completely blinded by database-specific parser differentials. The WAF only survived by falling back on a blind character-counting heuristic, a defense mechanism that is notorious for causing false positives in production environments.
